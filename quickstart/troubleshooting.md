...
It is the data plane of Mayastor. There are as many mayastor containers as many
storage nodes have been configured. This log file is useful when troubleshooting
IO errors but not only that. Management operations could also fail because of
...
These containers implement CSI spec for Kubernetes and run in the same pods
with moac and mayastor-csi containers. Although that they are not part of
Mayastor software, can contain useful information when Mayastor CSI
...
Coredump is a snapshot of process' memory with auxiliary information
...
The steps apply to Ubuntu OS. However different Linux distros would use
similar steps.
