...
When the reclaim policy is set to 'retain`, the underlying volume remains intact even after deletion of the PV, which leaves the volumes unusable. In this scenario, the external provisioner does not call the delete volume API. To overcome this, Mayastor deploys a PV garbage collector as part of the CSI controller. The garbage collector deletes the underlying volume object associated with the deleted PV with reclaim policy set to retain. 

  ### How does the PV garbage collector work? 
    A watcher object, deployed with PV Garbage, watches for PV resources continuously. When a PV is deleted, an event (generated by the kube api server) is received by the watcher object. These events are then validated. After which, the garbage collector initiates the delete operation on the Mayastor volumes using the Mayastor rest API. 